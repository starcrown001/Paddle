...
void FlashAttnV3BaseKernel(
  ...
  if (is_flashmask) {
    if(lt_start_row_indices.initialized())
      dynload::fa3_fwd_params_set_lt_start_ptr(params_handle, const_cast<int32_t *>(lt_start_row_indices.data<int32_t>()));
    else
      dynload::fa3_fwd_params_set_lt_start_ptr(params_handle, nullptr);

    if(lt_end_row_indices.initialized())
      dynload::fa3_fwd_params_set_lt_end_ptr(params_handle, const_cast<int32_t *>(lt_end_row_indices.data<int32_t>()));
    else
      dynload::fa3_fwd_params_set_lt_end_ptr(params_handle, nullptr);

    if(ut_start_row_indices.initialized())
      dynload::fa3_fwd_params_set_ut_start_ptr(params_handle, const_cast<int32_t *>(ut_start_row_indices.data<int32_t>()));
    else
      dynload::fa3_fwd_params_set_ut_start_ptr(params_handle, nullptr);

    if(ut_end_row_indices.initialized())
      dynload::fa3_fwd_params_set_ut_end_ptr(params_handle, const_cast<int32_t *>(ut_end_row_indices.data<int32_t>()));
    else
      dynload::fa3_fwd_params_set_ut_end_ptr(params_handle, nullptr);

    if(flashmask_maxmin.initialized())
      dynload::fa3_fwd_params_set_flashmask_maxmin_ptr(params_handle, const_cast<int32_t *>(flashmask_maxmin.data<int32_t>()));
    else
      dynload::fa3_fwd_params_set_flashmask_maxmin_ptr(params_handle, nullptr);

    dynload::fa3_fwd_params_set_h_flashmask(params_handle, startend_row_indices.dims()[1]);
    dynload::fa3_fwd_params_set_h_h_flashmask_ratio(params_handle, head_size / startend_row_indices.dims()[1]);
  } else {
    dynload::fa3_fwd_params_set_lt_start_ptr(params_handle, nullptr);
    dynload::fa3_fwd_params_set_lt_end_ptr(params_handle, nullptr);
    dynload::fa3_fwd_params_set_ut_start_ptr(params_handle, nullptr);

...
void FlashAttnV3GradBaseKernel(
      ...
      head_size_rounded <= 64
          ...
          : (head_size_rounded <= 96
                 ...
                 : (head_size_rounded <= 128
                        ? (is_causal || is_local || is_flashmask || softcap > 0.0 ? 64 : 80)
                        : 64));
  ...
  if (num_heads_k != num_heads &&
    ...
    DenseTensor flashmask_maxmin, lt_start_row_indices, lt_end_row_indices, ut_start_row_indices, ut_end_row_indices;
    if (is_flashmask) {
      PADDLE_ENFORCE_EQ(
      ...
      // TODO(umiswing): refine this block constraint (kBlockN % 32), since some of kBlockN is not divisible by 32
  ...
  if (is_flashmask) {
    if(lt_start_row_indices.initialized())
      dynload::fa3_bwd_params_set_lt_start_ptr(params_handle, const_cast<int32_t *>(lt_start_row_indices.data<int32_t>()));
    else
      dynload::fa3_bwd_params_set_lt_start_ptr(params_handle, nullptr);

    if(lt_end_row_indices.initialized())
      dynload::fa3_bwd_params_set_lt_end_ptr(params_handle, const_cast<int32_t *>(lt_end_row_indices.data<int32_t>()));
    else
      dynload::fa3_bwd_params_set_lt_end_ptr(params_handle, nullptr);

    if(ut_start_row_indices.initialized())
      dynload::fa3_bwd_params_set_ut_start_ptr(params_handle, const_cast<int32_t *>(ut_start_row_indices.data<int32_t>()));
    else
      dynload::fa3_bwd_params_set_ut_start_ptr(params_handle, nullptr);

    if(ut_end_row_indices.initialized())
      dynload::fa3_bwd_params_set_ut_end_ptr(params_handle, const_cast<int32_t *>(ut_end_row_indices.data<int32_t>()));
    else
      dynload::fa3_bwd_params_set_ut_end_ptr(params_handle, nullptr);

    if(flashmask_maxmin.initialized())
      dynload::fa3_bwd_params_set_flashmask_maxmin_ptr(params_handle, const_cast<int32_t *>(flashmask_maxmin.data<int32_t>()));
    else
      dynload::fa3_bwd_params_set_flashmask_maxmin_ptr(params_handle, nullptr);

    dynload::fa3_bwd_params_set_h_flashmask(params_handle, startend_row_indices.dims()[1]);
    dynload::fa3_bwd_params_set_h_h_flashmask_ratio(params_handle, num_heads / startend_row_indices.dims()[1]);
  } else {
    dynload::fa3_bwd_params_set_lt_start_ptr(params_handle, nullptr);
    dynload::fa3_bwd_params_set_lt_end_ptr(params_handle, nullptr);
    dynload::fa3_bwd_params_set_ut_start_ptr(params_handle, nullptr);
...
void FlashAttnV3GradKernel(const Context &ctx,
  ...
  if (is_mla) {
    *dv = Slice<T, Context>(ctx, dv_padded, {3}, {0}, {d_v});
    // TODO(umiswing): add head dimension check
  } else {
    *dv = dv_padded;
...
template <typename T, typename Context>
void FlashMaskV2GradKernel(const Context &ctx,
  const DenseTensor &q,
  const DenseTensor &k,
  const DenseTensor &v,
  const DenseTensor &out,
